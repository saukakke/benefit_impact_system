...
function validateId(array $data, string $field): int {
    if (!array_key_exists($field, $data) || filter_var($data[$field], FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]) === false) {
        jsonResponse(['success' => false, 'message' => "Invalid {$field}"], 422);
    }
    return (int) $data[$field];
}

function validateDateValue(array $data, string $field, bool $required = false): void {
    if (!array_key_exists($field, $data) || $data[$field] === null || $data[$field] === '') {
        if ($required) jsonResponse(['success' => false, 'message' => "Invalid {$field}"], 422);
        return;
    }
    $date = DateTimeImmutable::createFromFormat('!Y-m-d', (string) $data[$field]);
    if (!$date || $date->format('Y-m-d') !== $data[$field]) jsonResponse(['success' => false, 'message' => "Invalid {$field}"], 422);
}
